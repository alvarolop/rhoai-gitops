...
oc process -f prerequisites/ocp-nodes/template-gpu-worker.yaml \
    ...
    -p INSTANCE_TYPE="g5.4xlarge" | \
    oc apply -n openshift-machine-api -f -

GPU_NODE_NAME=$(oc get nodes -l node-role.kubernetes.io/gpu-worker='' -o jsonpath='{.items[0].metadata.name}')
oc adm taint node $GPU_NODE_NAME nvidia.com/gpu:NoSchedule

oc adm taint node $GPU_NODE_NAME nvidia.com/gpu-
----
...
[source, bash]
----
oc apply -f application-rhoai-configuration.yaml
----
