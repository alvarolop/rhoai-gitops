...
You can use the distributed workloads feature to queue, scale, and manage the resources required to run data science workloads across multiple nodes in an OpenShift cluster simultaneously. There are three components that take a relevant part in this feature:
...
These three components need to be enabled on the RHOAI installation configuration.
