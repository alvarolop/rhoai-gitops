...
[source, bash]
----
oc apply -f application-serve-vllm-mistral-7b.yaml
sleep 4
oc create secret generic hf-creds --from-env-file=hf-creds -n vllm-mistral-7b
----
...
openssl s_client -showcerts -connect granite-1b-a400m.granite-1b-a400m.svc.cluster.local:443 </dev/null

# Check models endpoint
curl --cacert /etc/pki/ca-trust/source/anchors/service-ca.crt https://granite-1b-a400m.granite-1b-a400m.svc.cluster.local:443/v1/models

# Check Completion (It might be /v1/chat/completions)
curl -s -X 'POST' https://granite-1b-a400m.granite-1b-a400m.svc.cluster.local/v1/completions -H 'Accept: application/json' -H 'Content-Type: application/json' -d '{"model": "granite-1b-a400m","prompt": "San Francisco is a"}'
----
...
cat application-open-webui.yaml | \
    ...
    LLM_INFERENCE_SERVICE_URL="https://granite-1b-a400m.granite-1b-a400m.svc.cluster.local" \
    envsubst | oc apply -f -
----
...
helm template components/open-webui --namespace="open-webui" \
    --set llmInferenceService.url="https://granite-1b-a400m.granite-1b-a400m.svc.cluster.local/v1" \
