...
* Change the image for the Node Feature Discovery container to the one for 4.20:
    ** In `./rhoai-dependencies/operator-nfd/nodefeaturediscovery-nfd-instance.yaml`, the `.spec.operand.image` field should have value `registry.redhat.io/openshift4/ose-node-feature-discovery-rhel9:v4.20`.
...
Uses the https://github.com/open-webui/helm-charts/tree/main/charts/open-webui[official Open WebUI Helm chart] with OpenShift OIDC authentication.

[source, bash]
----
# Configure .env file with required variables
cat > .env <<EOF
# OAuth Configuration for OpenShift OIDC
...
See link:open-webui-oidc-setup.md[Open WebUI OIDC Setup Guide] for more details.
